struct HMMpd {
	MatDoub a, b;
        VecInt obs, sr; // sr has the same dimension as obs, and tells if it is in the switching region (1) or not (0) 
	Int fbdone;
	Int mstat, nobs, ksym;
	Int lrnrm;
	...
	VecInt arnrm, brnrm;
	Doub BIG, BIGI, lhood;
        HMMpd(MatDoub_I &aa, MatDoub_I &bb, VecInt_I &obs, VecInt_I &sr);
	void forwardbackward2();
	void baumwelch();
	Doub loglikelihood() {return log(lhood)+lrnrm*log(BIGI);}
};

HMMpd::HMMpd(MatDoub_I &aa, MatDoub_I &bb, VecInt_I &obss, VecInt_I &srs) :
        a(aa), b(bb), obs(obss), sr(srs), fbdone(0),
...
void HMMpd::forwardbackward2() {
	...
	for (t=1; t<nobs; t++) {
		asum = 0;
		//if (sr[t]==1) { // switching region
		if (sr[t-1]==1) { // switching region
		  for (j=0; j<mstat; j++) {
		        sum = 0.;
		...
		} else {        // out of switching region
		  for (j=0; j<mstat; j++) {
		        sum = 0.;
			for (i=0; i<mstat; i++) {
			  if (i==j) {
			    sum += alpha[t-1][i]*1.0*b[j][obs[t]];
			  }
			}
			alpha[t][j] = sum;
			asum += sum;
	...
	for (t=nobs-2; t>=0; t--) {
		bsum = 0.;
		//if (sr[t+1]==1) { // in switching region
		if (sr[t]==1) { // in switching region
		  for (i=0; i<mstat; i++) {
			sum = 0.;
		  ...
		  for (i=0; i<mstat; i++) {
			...
			for (j=0; j<mstat; j++) {
			  if (j==i) {
			    sum += 1.0*b[j][obs[t+1]]*beta[t+1][j];
			  }
			}
			beta[t][i] = sum;
			bsum += sum;
