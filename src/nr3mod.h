...
{printf("ERROR: %s\n     in file %s at line %d\n", message,__FILE__,__LINE__); NRthrow(1);}
#else
struct NRerror {
	char *message;
	char *file;
...
#define NRthrow(message) NRthrow(NRerror(message,__FILE__,__LINE__));
...
#ifdef _CHECKBOUNDS_
if (i<0 || i>=nn) {
	NRthrow("NRvector subscript out of bounds");
}
#endif
	return v[i];
}
...
#ifdef _CHECKBOUNDS_
if (i<0 || i>=nn) {
	NRthrow("NRmatrix subscript out of bounds");
}
#endif
	return v[i];
}
